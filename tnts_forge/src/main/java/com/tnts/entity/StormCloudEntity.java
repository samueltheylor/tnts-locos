...
public class StormCloudEntity extends Entity {
    ...
    public void tick() {
        ...
        // rayos: cada 14 ticks, 3-5 rayos (MAS destructivo)
        if (age % 14 == 0) {
            int numBolts = 3 + serverLevel.random.nextInt(3);
            ...
            serverLevel.playSound(null, x, y, z, SoundEvents.LIGHTNING_BOLT_THUNDER,
                    SoundSource.WEATHER, 3.5F, 0.8F);
        }

        // viento: empuja entidades lejos (MAS fuerte)
        AABB box = new AABB(x - RADIUS, y - 12, z - RADIUS, x + RADIUS, y + 8, z + RADIUS);
        for (Entity e : serverLevel.getEntitiesOfClass(Entity.class, box,
            ...
            double strength = 0.20 * (1 - Math.min(1, dist / RADIUS)) + 0.03;
            e.setDeltaMovement(e.getDeltaMovement().add(dir.normalize().scale(strength)));
            e.hurtMarked = true;
        }

        // lluvia: particulas cayendo desde la nube (mas densa)
        for (int i = 0; i < 14; i++) {
